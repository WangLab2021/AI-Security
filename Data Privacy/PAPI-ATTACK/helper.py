...
class Helper:
    def __init__(self, current_time, params, name):
        ...
        self.adv_update_epoch = None
        self.shadow_indices = []
        self.poison_images = []
        self.poison_images_test = []
        self.poison_images_client = []
        self.train_indices = []
        self.test_indices= []
        self.client_indices = []
        self.ave_loss = []
    ...
    def get_one_vec(model, variable=False):
        ...
        if variable:
            ...
            sum_var = Variable(torch.FloatTensor(size).fill_(0))
        else:
    ...
    def average_shrink_models(self, weight_accumulator, target_model, epoch, exp_time):
        """
        Perform FedAvg algorithm and perform some clustering on top of it.

        """
        epoch_update = {}
        root = self.params['root_path']
        test_path = f'{root}/exp{exp_time}/test'
        try:
            os.mkdir(test_path)
        except FileExistsError:
        ...
        for name, data in target_model.state_dict().items():
            ...
            if not (self.params.get('tied',
                                 False) and name == 'decoder.weight' or '__' in name or 'running' in name or 'num_batches_tracked' in name):

                epoch_update[name] = copy.deepcopy(update_per_layer)
        each_update = []
        for key, tensor_val in epoch_update.items():
        ...
        torch.save(each_update, f'{test_path}/Gupdate_{epoch}_epoch.pkl')
        return True
