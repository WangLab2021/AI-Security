...
****parameter****
    ...
    l1: the lower boundary of the range
    l2: the upper boundary of the range
    l: the real location of user
*****return******
...
def obfuscate_location(distance, probability):
    ...
    for l1 in range(0, l, 1):
        for l2 in range(l, lmax, 1):
            ...
            for rands in range(1000):
                # print(i)
                obfuscatedDistance, prob = obfuscate_location(distance, probability)         # produce different obfuscatedDistance each time
                ...
                for j in range(2 ** TASKNUM):  # Task Allocation (TA) traverse
                    ...
                    lk2_temp = []
                    lk2_temp.append(lk2)
                    fakeCost = offloading_cost(e, vi, lk2_temp, K, weight3)  # Perturb location lk2
                    # fakeCost = offloading_cost(e,vi, lk2,K,weight3)     # Perturb location lk2
                    R = -fakeCost
                    if count == 0:  # run only once for ynitializing max_R value
                        ...
                        lk1_temp = []
                        lk1_temp.append(lk1)
                        cost = offloading_cost(e, vi, lk1_temp, K, weight3)  # Perturb location lk1
                        opt_cost = cost
