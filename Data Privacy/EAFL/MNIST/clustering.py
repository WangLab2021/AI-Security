...
def reclustering_kruscal(args,gradients_c):
    tag = []
    cluster = []
    #print("gradients:",gradients_c)
    gradients = do_flatten(gradients_c)
    father = [_ for _ in range(args.user_num)]
    g = []
    for i in range(args.user_num-1):
        j = i+1
        while j<args.user_num:
            g.append(tuple((i,j,spatial.distance.cosine(gradients[i],gradients[j]))))
            j+=1
    num_cluster = args.user_num
    ...
    for i in range(num_cluster):
        g = []
        for client in cluster[i]:
            g.append(gradients_c[client])
        s.append(similarity((g)))
    s.sort()
    ...
    if args.clustering == 'K-Means':
        ...
        if args.clustering == 'DBSCAN':
            ...
            if args.clustering == 'graph':
                tag_normal, cluster, num_cluster = reclustering_kruscal(args, gradients_c)
