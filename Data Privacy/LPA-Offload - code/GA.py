...
def matuingFuction(pop, p_Mating, server_num, p_Mutation, NUM):
    ...
    for i in range(int(len(a) / 2)):        #switch a[m]&a[n]
        ...
        x1 = list(a.pop())
        x2 = list(a.pop())
        x1,x2 = matuting(x1, x2, p1, p2)
        # variation
        x1 = variationFunction(x1, p_Mutation, server_num)
        x2 = variationFunction(x2, p_Mutation, server_num)
...
def variationFunction(list_a, p_Mutation, server_num):
    #Swap after determining a range
    if np.random.rand() < p_Mutation:
        variation_num = np.random.randint(1, int(len(list_a))/2)      #must change at least 1 point
        while variation_num>0:
