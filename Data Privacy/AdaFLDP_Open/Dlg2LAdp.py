...
# test client using adp local training method
def Vitrual_VulAdpClient(Epoch,device,model,dataset = DATAMODE, rd=0, eps = 0, channel = 3, mode='ori'):
    PRIVACY_MODE = mode
    seed = 1234  # 随机种子数为1234结果会较好
    #eps = 50000
    ...
    save_path = os.path.join(root_path, 'Adpresults/DLG_%s_%s_final' % (dataset,PRIVACY_MODE)).replace('\\', '/')
    lr = 0.1
    num_dummy = 1
    Iteration = 100
    num_exp = 1
    ...
    if not os.path.exists('Adpresults'):
        os.mkdir('Adpresults')
    ...
    for idx_net in range(num_exp):
        ...
        for method in ['DLG']:
            ...
            if original_dy_dx == []:
                print('original gradient is empty')
                return 0
            ...
            for iters in range(Iteration):
                ...
                if (iters+1) % 10 == 0 or iters ==0:
                    ...
                    if mses[-1] < min_MSE:
                        min_MSE = mses[-1]
                        final = dummy_data[0].cpu().detach().numpy()
                    ...
                    for imidx in range(num_dummy):
                        ...
                        for i in range(min(len(history), 14)):
                            ...
                            plt.axis('off')
                        if method == 'DLG':
                            plt.savefig('%s/DLG_on_%s_%s_%s_%s.png' % (save_path, rd, label_pred.item(), Epoch, opt))
                            plt.close()
                        elif method == 'iDLG':
        ...
        if dataset == 'mnist':
            # MNIST 需要去除归一化的效果
            #final=torch.clamp(torch.tensor(final*ds+dm),0,1)
            final=torch.tensor(final*ds+dm)
        np.save('./%s/%s_%s_%d_final'%(save_path,dataset,PRIVACY_MODE,rd),final)
        return mse_DLG