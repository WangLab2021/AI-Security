...
def save_checkpoint(
    state, is_best, is_best_dema, is_best_demo, args, result_dir, filename="checkpoint.pth.tar", save_dense=False
):
    torch.save(state, os.path.join(result_dir, filename))
    if is_best:
        shutil.copyfile(
            os.path.join(result_dir, filename),
            os.path.join(result_dir, "model_acc_best.pth.tar"),
        )

    if is_best_dema:
        shutil.copyfile(
            os.path.join(result_dir, filename),
            os.path.join(result_dir, "model_dema_best.pth.tar"),
        )

    if is_best_demo:
        shutil.copyfile(
            os.path.join(result_dir, filename),
            os.path.join(result_dir, "model_demo_best.pth.tar"),
        )
    ...
    if save_dense:
        ...
        if is_best:
            shutil.copyfile(
                os.path.join(result_dir, "checkpoint_dense.pth.tar"),
                os.path.join(result_dir, "model_acc_best_dense.pth.tar"),
            )
        if is_best_dema:
            shutil.copyfile(
                os.path.join(result_dir, filename),
                os.path.join(result_dir, "model_dema_best_dense.pth.tar"),
            )

        if is_best_demo:
            shutil.copyfile(
                os.path.join(result_dir, filename),
                os.path.join(result_dir, "model_demo_best_dense.pth.tar"),
            )
